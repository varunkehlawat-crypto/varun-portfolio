...
export default function GlyphPortal({
  ...
  useLayoutEffect(() => {
    ...
    const readInk = () => {
      ...
      target = candidates.find((c) => c.index === requested) ?? [...candidates].sort((a, b) => b.radius - a.radius || Math.abs(a.x - center.x) - Math.abs(b.x - center.x))[0] ?? null;
      return true;
    };
    ...
    const scheduleFrame = () => { if (!raf && active) raf = requestAnimationFrame(frame); };
    ...
    const visibility = new IntersectionObserver(([entry]) => {
      ...
      if (active) { dirty = true; scheduleFrame(); }
